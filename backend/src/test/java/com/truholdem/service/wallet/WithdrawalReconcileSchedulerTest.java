...
class WithdrawalReconcileSchedulerTest {
    ...
    private WithdrawalReconcileScheduler scheduler(AppProperties props) {
        ...
        return new WithdrawalReconcileScheduler(walletService, props, ethProvider, btcProvider);
    }
    ...
    void dispatchesByAsset() {
        ...
        when(walletService.withdrawalsForReview(WithdrawalStatus.BROADCAST))
                .thenReturn(List.of(ethW, ercW, btcW, tronW));
        ...
        // TRON has no coordinator → neither is called for it (only the 2 eth + 1 btc above)
    ...
    void disabledIsInert() {
        new WithdrawalReconcileScheduler(walletService, props(true, false), ethProvider, btcProvider)
                .reconcileBroadcast();

        verify(walletService, never()).withdrawalsForReview(any());
        verifyNoInteractions(eth, btc);
    }
}
