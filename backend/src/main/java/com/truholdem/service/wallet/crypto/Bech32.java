...
public final class Bech32 {
    ...
    private static final int BECH32_CONST = 1; // SegWit v0; v1+ would use 0x2bc830a3 (bech32m)

    private Bech32() {
    }

    /** Encode a SegWit address: {@code hrp} + witness version + program (e.g. "bc", 0, HASH160). */
    ...
    /** Decode a P2WPKH address for {@code hrp}; returns the 20-byte witness program, or null if invalid
     *  (bad charset, mixed case, wrong checksum, wrong hrp, non-v0, or wrong program length). */
    public static byte[] decodeP2wpkh(String hrp, String address) {
        ...
        if (dataLen < 1 || values[0] != 0) {
            return null; // only SegWit v0
        }
        int[] program5 = new int[dataLen - 1];
        System.arraycopy(values, 1, program5, 0, dataLen - 1);
        int[] program8 = convertBits(program5, 5, 8, false);
        if (program8 == null || program8.length != 20) {
            return null; // P2WPKH program is exactly 20 bytes
        }
    ...
    private static int[] createChecksum(String hrp, int[] data) {
        ...
        int pm = polymod(values) ^ BECH32_CONST;
